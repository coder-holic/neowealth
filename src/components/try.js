...
const groupByMonth = (docs, valueKey = null) => {
  // ... existing groupByMonth function (unchanged) ...
  const groups = {};
  docs.forEach((doc) => {
    const data = doc.data();
...
const Analytics = ({
  ...
  useEffect(() => {
    if (!user || !user.uid) {
      ...
      setIsLoading(false);
      if (onAnalyticsReady)
        onAnalyticsReady({ error: "User not authenticated" });
      return;
    }
    ...
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        const totalEarnings = convDocs.reduce(
          (sum, doc) => sum + (doc.data().commission || 0),
          0
        );
        const todayEarnings = todayConvDocs.reduce(
          (sum, doc) => sum + (doc.data().commission || 0),
          0
        );
    ...
    fetchAnalytics();
  }, [fromDate, toDate, user, onAnalyticsReady]);
